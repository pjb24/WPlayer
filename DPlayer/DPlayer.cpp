// DPlayer.cpp : 애플리케이션에 대한 진입점을 정의합니다.
//
...
void create_windows()
{
    ...
    for (auto it_adapter = _map_adapter.begin(); it_adapter != _map_adapter.end(); it_adapter++)
    {
        ...
        if (rect.right == _virtual_plus_window_left)
        {
            rect.right += _virtual_plus_window_right - _virtual_plus_window_left;
        }

        if (rect.left == _virtual_plus_window_right)
        {
            rect.left -= _virtual_plus_window_right - _virtual_plus_window_left;
        }
...
void config_setting()
{
    ...
    GetPrivateProfileString(L"DPlayer", L"log_file_size", L"1", result_w, 255, str_ini_path_w.c_str());
    _log_file_size = _ttoi(result_w);

    GetPrivateProfileString(L"DPlayer", L"log_file_rotation_count", L"3", result_w, 255, str_ini_path_w.c_str());
    ...
    GetPrivateProfileStringA("DPlayer", "default_texture_url", "", result_a, 255, str_ini_path_a.c_str());
    ...
    GetPrivateProfileString(L"DPlayer", L"virtual_plus_window_right", L"0", result_w, 255, str_ini_path_w.c_str());
    _virtual_plus_window_right = _ttoi(result_w);
}
